    ...
    App::new()
        ...
        .run(move |FrameContext { gfx, input, timer, .. }| {
            ...
            for touch in input.touches() {
                let world_pos = gfx.camera().screen_to_world(vec2(touch.location.x as f32, touch.location.y as f32));
                match touch.phase {
                    TouchPhase::Started => {
            ...
            for (idx, (id, path)) in touch_paths.iter().enumerate() {
                ...
                if path.len() >= 2 {
                    let mut builder = gfx.path().thickness(3.0).stroke_color(trail_color).begin(path[0]);
                    for &pt in &path[1..] {
                ...
                if let Some(&pos) = path.last() {
                    ...
                    gfx.polygon().segments(16).at(pos).radius(6.0).color(Color::WHITE);

                    let screen_pos = gfx.camera().world_to_screen(pos);
                    gfx.text(&format!("touch {}", id)).at(screen_pos + vec2(16.0, -16.0)).color(color);
                }
            }

            gfx.text(&format!("touches: {} | fps: {:.0}", input.touch_count(), timer.fps))
                .color(Color::WHITE);
        });
}
